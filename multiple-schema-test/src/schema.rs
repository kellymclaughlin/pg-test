...
pub fn run_threads(url: Arc<String>, thread_count: &u32, thread_iterations: Arc<u32>, schema_count: Arc<u32>) {
    ...
    for _number in 1..*thread_count {
        ...
        let h = thread::spawn(|| multiple_schema_queries(url_clone, thread_iterations_clone, schema_count_clone));
        handles.push(h);
    }
...
fn multiple_schema_queries(url: Arc<String>, thread_iterations: Arc<u32>, schema_count: Arc<u32>) -> HistogramPair {
    ...
    for _number in 1..*thread_iterations {
        ...
        let write_sql = "INSERT INTO ".to_string()
            + &schema_name
            + &".manta_bucket (key, bucket, vnode, owner, object_id, \
        ...
        let read_sql = "SELECT * FROM ".to_string()
            + &schema_name
            + &".manta_bucket WHERE owner = $1 AND bucket_id = $2 AND name = $3".to_string();
